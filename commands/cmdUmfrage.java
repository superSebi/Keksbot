...
public class cmdUmfrage implements Command, Serializable {
    ...
    private static EmbedBuilder getParsedPoll(Poll poll, Guild guild) {
        ...
        return new EmbedBuilder()
                .setAuthor(poll.getCreator(guild).getEffectiveName() + "'s Umfrage", null, poll.getCreator(guild).getUser().getAvatarUrl())
                .setDescription("<:umfrage:573535749190844417>  >> " + poll.heading + "\n\n" + ansSTR.toString())
                .setColor(Color.cyan);
    ...
    private void createPoll(String[] args, MessageReceivedEvent event, boolean secret) {
        ...
        if (polls.containsKey(event.getGuild())) {
            message("❌ | Derzeit läuft schon eine Umfrage!", Color.red);
            return;
        }
        ...
        Message msg = channel.sendMessage("Erstellte Umfrage:").complete();
    ...
    private static void addVote(Guild guild, Member author, int voteIndex) {
        ...
        if (poll.votes.containsKey(author.getUser().getId())) {
            tempList.get(guild).getTextChannel().sendMessage(new EmbedBuilder().setColor(Color.red).setDescription("Sorry, " + author.getAsMention() + ", du kannst nur einmal voten!").build()).queue(m ->
    ...
    private void statsPoll(MessageReceivedEvent event) {
        ...
        if (!polls.containsKey(event.getGuild())) {
            message("❌ | Derzeit läuft keine Umfrage!", Color.red);
            return;
        }
        ...
        if (poll.secret && !poll.getMessage(g).getTextChannel().equals(event.getTextChannel())) {
            message("❌ | Diese Umfrage ist geheim!", Color.red);
            return;
        }
    ...
    private void closePoll(MessageReceivedEvent event) {
        ...
        if (poll.secret && !poll.getMessage(g).getTextChannel().equals(event.getTextChannel())) {
            message("❌ | Die Umfrage ist geheim!", Color.red);
            return;
        }

        if (!poll.getCreator(g).equals(event.getMember()) && !event.getMember().hasPermission(Permission.KICK_MEMBERS)) {
            message("❌ | Nur der Ersteller (" + poll.getCreator(g).getAsMention() + ") kann sie schließen!", Color.red);
            return;
        }
        ...
        tempList.remove(g);
        channel.sendMessage(getParsedPoll(poll, g).build()).queue();
        message("<:umfrage:573535749190844417> | Umfrage geschlossen von " + event.getAuthor().getAsMention() + ".", new Color(0xFF7000));
    ...
    public void action(String[] args, MessageReceivedEvent event) throws ParseException, IOException {
        ...
        switch (args[0]) {
            ...
            case "secret":
                ...
                message("Die Erstellte Umfrage ist **geheim** und kann nur in **diesem Channel** gesehen werden.", Color.yellow);
                break;
            case "stats":
                statsPoll(event);
                break;
